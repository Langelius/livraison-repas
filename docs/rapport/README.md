# Rapport de projet
 « Brouillon de travail — le rapport final formaté sera déposé dans /livrables »

Ce rapport est rédigé **au fil de l'eau**, pas à la fin. Chaque section est
complétée au fur et à mesure de l'avancement.

## 1. Présentation du projet
[À compléter]

## 2. Objectifs
[À compléter]

## 3. Analyse des besoins
[À compléter]

## 4. Méthodologie
[À compléter — voir la convention de collaboration et le workflow Git]

## 5. Difficultés rencontrées
[À compléter en continu]

## 6. Solutions apportées
[À compléter en continu]

## 7. Résultats obtenus
[À compléter]

## 8. Perspectives d'amélioration
Application livreur, application admin, chat client/livreur, modération,
gestion des pourboires.
